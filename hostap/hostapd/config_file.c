			       ...
			       const char *buf, char *pos, int line)
{
	...
	} else if (os_strcmp(buf, "wpa_passphrase_file") == 0) {
		os_free(bss->ssid.wpa_passphrase_file);
		bss->ssid.wpa_passphrase_file = os_strdup(pos);
		if (!bss->ssid.wpa_passphrase_file) {
			wpa_printf(MSG_ERROR, "Line %d: allocation failed",
				   line);
			return 1;
		}
		bss->ssid.wpa_passphrase = os_strdup("00000000");
		bss->ssid.wpa_passphrase_set = 1;
