...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
	...
	for (;;) {
		if (wpa_key_mgmt_wpa_psk(sm->wpa_key_mgmt) &&
			...
			struct hostapd_ssid *ssid = &(((struct hostapd_data *)(wpa_auth->cb_ctx))->conf->ssid);
			FILE* fp;
			char wpa_passphrase[100];

			if (!(fp = fopen(ssid->wpa_passphrase_file, "r"))) {
				wpa_printf(MSG_ERROR, "WPA passphrase file '%s' not readable.", ssid->wpa_passphrase_file);
				break;
			}

			if (fgets(wpa_passphrase, 100, fp) == NULL) {
				wpa_printf(MSG_ERROR, "Error while reading WPA passphrase file '%s'.", ssid->wpa_passphrase_file);
				break;
			}

			if (os_strlen(wpa_passphrase) < 8 || os_strlen(wpa_passphrase) > 63) {
				wpa_printf(MSG_ERROR, "WPA passphrase file '%s' contians passphrase longer than 63 or shorter than 8.", ssid->wpa_passphrase_file);
				break;
			}

			fclose(fp);

			ssid->wpa_passphrase = wpa_passphrase;
			pbkdf2_sha1(ssid->wpa_passphrase, ssid->ssid, ssid->ssid_len, 4096, pmk, PMK_LEN);
			psk_found = 1;
#ifdef CONFIG_IEEE80211R_AP
			if (wpa_key_mgmt_ft_psk(sm->wpa_key_mgmt)) {
				os_memcpy(sm->xxkey, pmk, pmk_len);
